...
export function FieldsPane({ aiConfidence, aiFieldNotes, autosave }: Props) {
  const {
    register,
    formState: { errors },
  ...
  return (
    <section className="bg-white border border-border rounded-md p-4 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Field label="대회명" required note={aiFieldNotes?.name} error={errors.name?.message}>
          <Input {...register("name")} />
        </Field>

        <Field label="카테고리" required note={aiFieldNotes?.category} error={errors.category?.message}>
        ...
        <Field label="주최" required note={aiFieldNotes?.host} error={errors.host?.message}>
          <Input {...register("host")} />
        </Field>

        <Field label="회차" note={aiFieldNotes?.edition} error={errors.edition?.message}>
          <Input {...register("edition")} placeholder="예: 제8회" />
        </Field>

        <Field label="시작일" required note={aiFieldNotes?.dateStart} error={errors.dateStart?.message}>
          <Input type="date" {...register("dateStart")} />
        </Field>

        <Field label="종료일" required note={aiFieldNotes?.dateEnd} error={errors.dateEnd?.message}>
          <Input type="date" {...register("dateEnd")} />
        </Field>

        <Field label="접수 시작" note={aiFieldNotes?.registrationStart} error={errors.registrationStart?.message}>
          <Input type="date" {...register("registrationStart")} />
        </Field>

        <Field label="접수 마감" required note={aiFieldNotes?.registrationEnd} error={errors.registrationEnd?.message}>
          <Input type="date" {...register("registrationEnd")} />
        </Field>

        <Field label="장소" required note={aiFieldNotes?.venue} error={errors.venue?.message}>
        ...
        <Field label="참가비" note={aiFieldNotes?.fee} error={errors.fee?.message}>
